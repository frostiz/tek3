var Weather = React.createClass({
    ...
    componentDidMount: function() {
        ...
        _this.props.loadWidgets("weather", "/loadWidgets", function(data) {
    ...
    getParam: function(widgetName, params, id) {
        let array = [
            ['get-city-temperature', {city: params.city, id: id}]
        ];
        for (var i in array) {
            if (array[i][0] === widgetName) {
                return array[i][1];
...
var GetCityTemperatureFields = React.createClass({
    statics: {
        getFields: function() {
            return JSON.stringify({
                city: 'weatherSearchBar'
            });
        }
    },
    render: function() {
        return (
            <div>
                <div>
                    <input className="au-input w-100" id="weatherSearchBar" placeholder="Search for city weather..." type="text"/>
                </div>
            </div>
        );
    }
...
var WeatherForm = React.createClass({
    loadFields: function() {
        let array = [
            ["get-city-temperature", <GetCityTemperatureFields />]
        ];
    ...
    getInitialState: function () {
        return ({option: "get-city-temperature"})
    },
    componentDidMount: function() {
    ...
    addWidget: function() {
        let array = [
            ["get-city-temperature", GetCityTemperatureFields.getFields()],
        ];
        for (var i in array) {
            if (array[i][0] === this.state.option) {
                ...
                axios.post(this.props.baseUrl + "/addWidget",
                    ...
                    .then(function (response) {
                        ...
                        _this.props.updateWeatherState(response.data);
                    })
        ...
        return (
            <div>
                <select className="selectWidget m-b-10" onChange={this.handleSelect}>
                    <option value="get-city-temperature">Get the temperature of a city</option>
                </select>
